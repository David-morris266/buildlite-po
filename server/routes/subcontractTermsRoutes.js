...
function authoritativeBody(req){return {...(req.body||{}),actor:actorFromAuth(req.buildliteAuth,req.requiredPermission).actor};}
...
router.post('/versions/:id/publish',requirePermission(PERMISSIONS.TERMS_PUBLISH),async(req,res)=>{const c=await tenant(res);if(c)send(res,await repo.publish(c.id,req.params.id,authoritativeBody(req)),'version');});
...
router.put('/default',requirePermission(PERMISSIONS.TERMS_DEFAULT),async(req,res)=>{const c=await tenant(res);if(c)send(res,await repo.setTenantDefault(c.id,req.body.termsVersionId,authoritativeBody(req)));});
