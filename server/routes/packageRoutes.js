...
router.post("/:packageId/certificates/:certificateId/submit", requirePermission(PERMISSIONS.CERTIFICATE_SUBMIT), async (req, res) => {
  try {
    if (!isDbConfigured()) {
      return res.status(500).json({ message: "Database not configured" });
    }
    ...
    const result = await submitCertificateForPackage(
      ...
      req.params.certificateId,
      body,
      { actor: actorFromAuth(req.buildliteAuth, PERMISSIONS.CERTIFICATE_SUBMIT).actor, auth: req.buildliteAuth }
    );
    if (!result.ok) return sendCertificateError(res, result);
    res.json(result.certificate);
  } catch (err) {
...
router.post("/:packageId/certificates/:certificateId/reject", requirePermission(PERMISSIONS.CERTIFICATE_LOCK), async (req, res) => {
  try {
    if (!isDbConfigured()) {
      return res.status(500).json({ message: "Database not configured" });
    }
