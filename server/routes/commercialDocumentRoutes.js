...
router.post('/packages/:packageId/certificates/:certificateId/payment-notice',async(req,res)=>{try{const client=await active(res);if(client)send(res,await repo.generateNoticeDocument(client,req.params.packageId,req.params.certificateId,'payment_notice',req.body),201);}catch(e){res.status(500).json({message:e.message});}});
router.post('/packages/:packageId/certificates/:certificateId/combined-certificate-payment-notice',async(req,res)=>{try{const client=await active(res);if(client)send(res,await repo.generateNoticeDocument(client,req.params.packageId,req.params.certificateId,'combined_certificate_payment_notice',req.body),201);}catch(e){res.status(500).json({message:e.message});}});
