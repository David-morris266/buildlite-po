...
router.post('/combined-approvals',requirePermission(PERMISSIONS.CERTIFICATE_LOCK),requirePermission(PERMISSIONS.PAYMENT_AUTHORITY_APPROVE),async(req,res)=>{try{const result=await repository.approveSubmittedCertificate(clientId(req),req.body||{},req.buildliteAuth);res.status(result.status).json(result.ok?result:{message:result.message});}catch(error){res.status(error.status||500).json({message:error.message||'Failed to complete commercial approval.'});}});
