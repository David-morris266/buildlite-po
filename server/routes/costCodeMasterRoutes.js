...
const hierarchyWorksheet = require('../services/costCodeHierarchyWorksheetRepository');
const {
...
router.get('/hierarchy-worksheet',requirePermission(PERMISSIONS.COMMERCIAL_STRUCTURE_MANAGE),async(req,res)=>{try{const client=await withActiveClient(req,res);if(!client)return;const result=await hierarchyWorksheet.exportWorksheet(client.id,req.buildliteAuth);if(!result.ok)return res.status(result.status||400).json({message:result.message});return res.json({worksheet:result.document});}catch(error){return res.status(error.status||500).json({message:error.message||'Failed to export hierarchy worksheet.'});}});
router.post('/hierarchy-worksheet/preview',requirePermission(PERMISSIONS.COMMERCIAL_STRUCTURE_MANAGE),async(req,res)=>{try{const client=await withActiveClient(req,res);if(!client)return;const result=await hierarchyWorksheet.preview(client.id,req.body||{},req.buildliteAuth);if(!result.ok)return res.status(result.status||400).json({message:result.message,errors:result.errors});return res.json({preview:result.preview});}catch(error){return res.status(error.status||500).json({message:error.message||'Failed to preview hierarchy worksheet.'});}});
router.post('/hierarchy-worksheet/apply',requirePermission(PERMISSIONS.COMMERCIAL_STRUCTURE_MANAGE),async(req,res)=>{try{const client=await withActiveClient(req,res);if(!client)return;const result=await hierarchyWorksheet.apply(client.id,req.body||{},req.buildliteAuth);if(!result.ok)return res.status(result.status||400).json({message:result.message,errors:result.errors});return res.json({summary:result.summary});}catch(error){return res.status(error.status||500).json({message:error.message||'Failed to apply hierarchy worksheet.'});}});
