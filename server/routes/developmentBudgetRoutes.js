...
router.post('/budget-authority/site-start-budget',requirePermission(PERMISSIONS.DEVELOPMENT_BUDGET_POST),async(req,res)=>{try{const result=await repo.confirmSiteStartBudget(req.buildliteAuth.clientId,req.params.developmentId,req.body||{},req.buildliteAuth);res.status(result.status).json(result.ok?result:{message:result.message});}catch(error){res.status(error.status||500).json({message:error.message||'Failed to confirm Site Start Budget.'});}});
