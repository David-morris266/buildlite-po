...
router.post('/packages/:packageId/certificates/:certificateId/intended-payments',requirePermission(PERMISSIONS.INTENDED_PAYMENT_CONFIRM),async(req,res)=>{try{const c=await active(res);if(c)send(res,await repo.createDecision(c.id,req.params.packageId,req.params.certificateId,{...req.body,actor:actorFromAuth(req.buildliteAuth,PERMISSIONS.INTENDED_PAYMENT_CONFIRM).actor},{auth:req.buildliteAuth}),201);}catch(e){res.status(e.status||500).json({message:e.message});}});
...
router.post('/payment-notices/:noticeId/issue',async(req,res)=>{try{const c=await active(res);if(c)send(res,await repo.issue(c.id,req.params.noticeId,{...req.body,actor:req.buildliteAuth.displayName},{auth:req.buildliteAuth}));}catch(e){res.status(e.status||500).json({message:e.message});}});
