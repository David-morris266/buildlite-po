...
function costCodeRowToDocument(row) {
  ...
  return {
    ...
    commercialHead: row.resolved_commercial_head || row.commercial_head || null,
    commercialHeadId: row.commercial_head_id || null,
    commercialFamily: row.resolved_commercial_family || row.commercial_family || null,
    commercialFamilyId: row.commercial_family_id || null,
    canonicalReportingGroup: row.resolved_reporting_group || row.reporting_group || null,
    reportingGroupId: row.reporting_group_id || null,
    reportingGroup: row.resolved_reporting_group || reportingGroup,
    trade: row.resolved_reporting_group || reportingGroup,
    ...
    hierarchyReviewDisposition: row.hierarchy_review_disposition || null,
    hierarchyReviewState: row.hierarchy_resolution_state || null,
    hierarchyReviewedAt: toIso(row.hierarchy_reviewed_at),
    hierarchyReviewedBy: row.hierarchy_reviewed_by_display_name || null,
    importEvidence: Array.isArray(row.import_evidence) ? row.import_evidence : [],
