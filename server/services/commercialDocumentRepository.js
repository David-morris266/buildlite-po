...
async function generateNoticeDocument(client,packageId,certificateId,documentType,body={},options={}){
  ...
  const db=await pool.connect();try{await db.query('BEGIN');
    ...
    if(documentType==='pay_less_notice'){const payLess=(await db.query(`SELECT n.status,s.id snapshot_id FROM package_payment_notices n JOIN package_payment_notice_snapshots s ON s.notice_id=n.id AND s.client_id=n.client_id AND s.stage='issued' WHERE n.client_id=$1 AND n.id=$2 AND n.source_notice_id=$3 AND n.package_id=$4 AND n.certificate_id=$5 AND s.id=$6 FOR UPDATE OF n`,[client.id,built.sourceSnapshotIds.payLessNoticeId,built.sourceSnapshotIds.paymentNoticeId,packageId,certificateId,built.sourceSnapshotIds.payLessIssuedSnapshotId])).rows[0];if(!payLess||payLess.status!=='issued'){await db.query('ROLLBACK');return fail(409,'Issued Pay Less authority changed before document generation completed.');}}
...
async function issueDocument(clientId,documentId,body={}){const db=await pool.connect();try{await db.query('BEGIN');const row=(await db.query(`SELECT * FROM commercial_documents WHERE client_id=$1 AND id=$2 FOR UPDATE`,[clientId,documentId])).rows[0];if(!row){await db.query('ROLLBACK');return fail(404,'Document not found.');}if(row.status!=='generated'){await db.query('ROLLBACK');return fail(409,'Only a Generated document can be Issued.');}
  ...
  if(row.document_type!=='payment_certificate'){const snapshotId=row.source_snapshot_ids?.paymentNoticeIssuedSnapshotId;const noticeId=row.source_snapshot_ids?.paymentNoticeId;const notice=(await db.query(`SELECT 1 FROM package_payment_notices n JOIN package_payment_notice_snapshots s ON s.notice_id=n.id AND s.client_id=n.client_id AND s.stage='issued' WHERE n.client_id=$1 AND n.id=$2 AND n.status='issued' AND s.id=$3`,[clientId,noticeId,snapshotId])).rows[0];if(!notice){await db.query('ROLLBACK');return fail(409,'Source Issued Payment Notice authority is no longer valid.');}if(row.document_type==='pay_less_notice'){const payLess=(await db.query(`SELECT 1 FROM package_payment_notices n JOIN package_payment_notice_snapshots s ON s.notice_id=n.id AND s.client_id=n.client_id AND s.stage='issued' WHERE n.client_id=$1 AND n.id=$2 AND n.status='issued' AND n.source_notice_id=$3 AND s.id=$4`,[clientId,row.source_snapshot_ids?.payLessNoticeId,noticeId,row.source_snapshot_ids?.payLessIssuedSnapshotId])).rows[0];if(!payLess){await db.query('ROLLBACK');return fail(409,'Source Issued Pay Less authority is no longer valid.');}}}
