/**
 * BL-033D.1 — Pure TIME / LUMP_SUM proposal calculations.
...
function resolveBasisDate(basis, fixedDate, programme = {}) {
  ...
  if (key === TIME_BASES.SITE_START) {
    return toIsoDate(programme.siteStart) || null;
  }
  if (key === TIME_BASES.FIRST_COMPLETION) {
    return toIsoDate(programme.firstCompletion) || null;
  }
  if (key === TIME_BASES.FINAL_COMPLETION) {
    return toIsoDate(programme.finalCompletion) || null;
  }
  if (key === TIME_BASES.FIXED_DATE) {
    return toIsoDate(fixedDate) || null;
  }
  return null;
}
...
function unresolvedReasonForBasis(basis, kind) {
  ...
  if (key === TIME_BASES.FIRST_COMPLETION) {
    return PRELIMS_UNRESOLVED_REASONS.MISSING_FIRST_COMPLETION;
  }
  if (key === TIME_BASES.FINAL_COMPLETION) {
    return PRELIMS_UNRESOLVED_REASONS.MISSING_FINAL_COMPLETION;
  }
  if (key === TIME_BASES.FIXED_DATE) {
    return kind === "start"
...
function elapsedCalendarMonths(startDate, endDate, reportingMonth) {
...
function calculateLumpSumLine(line = {}) {
...
function calculatePrelimsLine(line = {}, context = {}) {
  const driver = String(line.forecastDriver || "").trim();
  if (driver === PRELIMS_DRIVERS.TIME) {
    return calculateTimeLine(line, context);
  }
  if (driver === PRELIMS_DRIVERS.LUMP_SUM) {
    return calculateLumpSumLine(line);
  }
  return invalid(PRELIMS_UNRESOLVED_REASONS.INVALID_AMOUNT);
}
...
function aggregatePrelimsLines(lines = []) {
  ...
  const developmentActiveProposal = moneyOrNull(resolvedProposalParts, {
...
function suggestedPrelimsDriver(classificationDriver) {
