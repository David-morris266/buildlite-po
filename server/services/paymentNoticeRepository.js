...
async function transition(clientId,noticeId,actionName,body={},options={}){const db=await pool.connect();try{await db.query('BEGIN');const row=(await db.query(`SELECT * FROM package_payment_notices WHERE client_id=$1 AND id=$2 FOR UPDATE`,[clientId,noticeId])).rows[0];if(!row){await db.query('ROLLBACK');return fail(404,'Notice not found.');}if(actionName==='issue'){const p=row.notice_type==='pay_less_notice'?require('../auth/permissions').PERMISSIONS.PAY_LESS_ISSUE:require('../auth/permissions').PERMISSIONS.PAYMENT_NOTICE_ISSUE;require('../auth/authorization').assertServicePermission(options.auth,p);}
...
async function createDecisionAuthorized(clientId,packageId,certificateId,body={},options={}){
  if(body.confirm===true)require('../auth/authorization').assertServicePermission(options.auth,require('../auth/permissions').PERMISSIONS.INTENDED_PAYMENT_CONFIRM);
  return createDecision(clientId,packageId,certificateId,body);
}
module.exports={getWorkspace,createPaymentNotice,patchDraft,prepare:(c,id,b,o)=>transition(c,id,'prepare',b,o),issue:(c,id,b,o)=>transition(c,id,'issue',b,o),createDecision:createDecisionAuthorized,createPayLess};
