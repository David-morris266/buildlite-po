/**
 * BL-011D.02A — Capture compact valuation grid screenshots.
 */
const fs = require("fs");
...
async function main() {
  ...
  await page.screenshot({
    path: path.join(OUT_DIR, "BL-011D-02A-compact-valuation-grid.png"),
    fullPage: false,
  });
  ...
  await page.screenshot({
    path: path.join(OUT_DIR, "BL-011D-02A-detail-panel.png"),
    fullPage: false,
  });

  await page.click(".po-cert-grid__toolbar-btn:nth-child(4)");
  await new Promise((r) => setTimeout(r, 400));
  await page.screenshot({
    path: path.join(OUT_DIR, "BL-011D-02A-bulk-toolbar-history.png"),
    fullPage: false,
  });

  await browser.close();
  console.log("BL-011D.02A screenshots saved to docs/screenshots/");
}
