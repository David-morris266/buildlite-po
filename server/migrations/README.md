...
| `016_client_prelims_templates.sql` | BL-033D.x.1: tenant-owned company Prelims templates + lines (additive). COMPLETE. Applied on `buildlite_test` and local `buildlite_clone`. Company-template UAT **PASSED**. Does not alter CVR, snapshots, programme, classification, or `development_prelims_items`. BuildLite Standard is a product-owned application definition, not tenant rows. Multiple template lines may map to the same customer cost code (no unique on `cost_code_key`). |
| `017_cost_codes_tenant_master.sql` | BL-033D.x.2A.1 IMPLEMENTED AND BANKED: additive Admin-master columns on existing `cost_codes`. Unique `(client_id, lower(btrim(code)))`. No backfill. Applied on `buildlite_test` by tests. Clone collision check under `lower(btrim(code))`: **0**. **Do not apply to `buildlite_clone` until x.2A.3.** Not an Admin authority cutover. |
