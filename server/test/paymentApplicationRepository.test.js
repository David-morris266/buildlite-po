...
test('server persistence enforces the operands required by each application basis',async(t)=>{if(!isDbConfigured())return t.skip('TEST_DATABASE_URL not configured');const fixture=await seed('V');const cases=[
  [{applicationBasis:'current_period_gross',currentPeriodGrossClaimed:''},/current-period gross/i],
  [{applicationBasis:'cumulative_less_previous_application',currentPeriodGrossClaimed:null,cumulativeGrossClaimed:100,previousApplicationStated:''},/previous application/i],
  [{applicationBasis:'cumulative_less_previous_certified',currentPeriodGrossClaimed:null,cumulativeGrossClaimed:100,previousCertifiedStated:''},/contractor\/application/i],
  [{applicationBasis:'net_only',currentPeriodGrossClaimed:null,netRequestedStated:''},/net amount/i],
];for(const [overrides,message] of cases){const result=await repo.createApplication(fixture.client.id,fixture.pkg.id,body(fixture,{applicationReference:`APP-${randomUUID()}`,...overrides}));assert.equal(result.status,400);assert.match(result.message,message);}
const net=await repo.createApplication(fixture.client.id,fixture.pkg.id,body(fixture,{applicationReference:'APP-NET',applicationBasis:'net_only',currentPeriodGrossClaimed:null,netRequestedStated:900}));assert.equal(net.ok,true,net.message);assert.equal(net.application.comparison.comparable,false);assert.equal(net.application.comparison.applicationCurrentGross,null);});
