...
8. QS expected-liability treatment and other forecast adjustments remain visible, auditable, and must not rewrite the underlying fact. **HD-001 resolved:** submitted CE expected liability defaults to full submitted value and feeds **Final Forecast**, not System Forecast. **Not implemented yet.**
...
1. **NEXT:** **BL-038A — CE-linked expected liability design preflight** (docs/design only until approved). Do **not** implement expected liability, Detailed Selling Costs, Hawthorn import, or a Hawthorn remap in the next slice. Keep P04 Draft. Do **not** Submit or Approve & Lock P04. Do **not** create P05. Do **not** Adopt UAT-CC-001. **BL-034D is BANKED** at `6d11491` (human UAT **PASS**; Test Site 1 5400 adopted **£182,780.64**). Founding Product Principles recorded. **HD-001 RESOLVED** by owner (default submitted CE expected liability = full submitted value → Final Forecast). **HD-002 and HD-008 remain resolved.** Hawthorn Gardens is retained as a future client-chart known-answer UAT. Do **not** switch 5231 to TIME. Do **not** Save migrated Admin cost-code rows whose server `reporting_group` is absent from the local Commercial Structure catalog. Historic P01/P02/P03 `reporting_month` remains NULL. P04 remains Draft with `reporting_month` **2026-08**; 5231 adj **+£7,720** / accrual **£120**.
...
The backbone diagram above is **original design intent**. **HD-001** now settles: Potential Liability stays visible; System Forecast stays approved facts; expected liability feeds Final Forecast. Payment Notice / Pay Less is not implemented. Built vs deferred is in the constitution, not in this spine.
