...
**Current position:** branch `buildlite-V1-1`, Doc 67 persistence programme. Developments, packages, Commercial Events, order matrices and V1 payment certificates are server-authoritative when their local authority flags are ON. **BL-030 is fully complete** (historical-freeze UAT passed). **BL-031B** is implemented (client CVR/ledger cache and hydration); **BL-031 is not complete** and runtime CVR/ledger authority remains localStorage. **NEXT after bank is BL-031C.** See `CURRENT_STATE.md` and `docs/test-data/README.md`.
