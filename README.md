...
**Current position:** branch `buildlite-V1-1`, Doc 67 persistence programme. Developments, packages, Commercial Events, order matrices and V1 payment certificates are server-authoritative when their local authority flags are ON. **BL-030 is fully complete** (historical-freeze UAT passed). **BL-031A** is the CVR + purchase ledger **server persistence/API foundation only** (BL-031 is not complete). **NEXT after bank/migrate is BL-031A.1 local clone migration.** See `CURRENT_STATE.md` and `docs/test-data/README.md`.
