 ...
 * Compares resolved Prelims proposal totals to live CVR rows at cost-code grain.
 */

import { calculatePrelimsLine, roundMoney as roundMoneyEngine } from './prelimsForecastEngine.js';
import {
  PRELIMS_CALC_STATES,
  PRELIMS_STATUSES,
} from './prelimsConstants.js';

export const PRELIMS_ADOPTION_METADATA_KEY = 'prelimsAdoption';

export const PRELIMS_ADOPTION_DRIFT_STATES = {
...
export const PRELIMS_ADOPTION_FLAG_KEYS = {
...
/**
 * Deterministic proposal fingerprint for drift detection.
 * Includes development, period, reporting month, and resolved line inputs.
 */
export function buildProposalFingerprint({
  developmentId = '',
  periodKey = '',
  reportingMonth = null,
  lines = [],
...
export function normalizePrelimsAdoptionMetadata(raw) {
...
export function buildPrelimsAdoptionMetadata({
  adoptedTargetFinal,
  adoptedAdjustment,
...
export function extractPrelimsAdoptionMetadata(displayMetadata) {
...
export function enrichPrelimsItemsForAdoption(items = [], context = {}) {
...
/**
 * Aggregate resolved ACTIVE totals and unresolved exposure by cost code.
 */
export function aggregateAdoptionCandidatesByCostCode(enrichedLines = []) {
...
export function resolveAdoptionDriftState({
  metadata = null,
  currentFingerprint = '',
...
export function comparePrelimsAdoptionCandidate({
  costCodeKey,
  prelimsBucket = null,
  cvrRow = null,
  classification = null,
...
export function buildPrelimsAdoptionPreview({
  developmentId = '',
  periodKey = '',
  reportingMonth = null,
  prelimsItems = [],
