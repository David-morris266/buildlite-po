...
export function buildCvrTotals(rows) {
  return {
    ...
    recognisedObligation: sumNullable(rows.map((row) => row.recognisedObligation)),
    uncommittedForecast: sumNullable(rows.map((row) => row.uncommittedForecast)),
    changeExposure: sumNullable(rows.map((row) => row.changeExposure)),
