...
export default function DevelopmentBudgetWorkspace({ developmentId, siteStartDate = '' }) {
  ...
  async function confirmSiteStart() {
    if (!siteStart.approvedEffectiveDate || !siteStart.reference.trim() || !siteStart.approvalReason.trim()) { setError('Enter the approved baseline date, reference and approval basis.'); return; }
    setSaving(true); setError(''); setSuccess('');
    try { await confirmSiteStartBudget(developmentId, siteStart); setSuccess('Site Start Budget confirmed.'); setSiteStartOpen(false); await load(); }
    catch (caught) { setError(caught.message); } finally { setSaving(false); }
  }
