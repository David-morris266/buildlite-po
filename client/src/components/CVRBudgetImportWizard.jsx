...
export default function CVRBudgetImportWizard({
  ...
  const validationContext = useMemo(
    ...
    [development.id, knownCostCodes]
  );

  const validationResult = useMemo(() => {
    if (!parsed) return null;
    return validateBudgetImport({ ...parsed, fieldByColumn }, validationContext);
  }, [parsed, fieldByColumn, validationContext]);

  const previewRows = useMemo(() => {
    if (!parsed) return [];
    return buildBudgetImportPreview(parsed.rows, parsed.headerRowIndex, fieldByColumn, 5);
  ...
  async function handleImport() {
    ...
    setError('');
    try {
      const result = await executeBudgetImport(development.id, validationResult, { periodKey });
      if (!result?.ok) {
        setError(result?.errors?.[0] || result?.message || 'Import failed.');
        return;
      }
      setComplete(result);
        ...
        <section className="po-module-card po-import-step">
          <dl className="po-import-review-grid dev-ledger-import__summary">
            ...
            <div>
              ...
              <dd>{complete.created ?? 0}</dd>
            </div>
            <div>
              <dt>Updated</dt>
              <dd>{complete.updated ?? 0}</dd>
            </div>
            <div>
  ...
  const masterError = formatBudgetImportMasterError(validationResult?.unknownCodes);
      ...
      {step.id === 'validation' && validationResult ? (
        <>
          <section className="po-module-card po-import-step">
            ...
            {masterError ? (
              <p className="dev-ledger-import__blocked" role="alert" style={{ whiteSpace: 'pre-wrap' }}>
                {masterError}
              </p>
            ) : null}
        ...
        <>
          <section className="po-module-card po-import-step">
            ...
            <p className="po-import-step__lead">
              ...
              {validationResult.importedCount === 1 ? '' : 's'} into the Draft CVR for this
              development. The import is applied in full or not at all.
            </p>
          </section>
          <div className="po-import-step__actions">
            ...
            <button
              type="button"
              className="po-btn-primary"
              disabled={processing || !validationResult.canImport}
              onClick={handleImport}
            >
              {processing ? 'Importing…' : 'Import Budget'}
            </button>
            <button type="button" className="po-import-step__cancel" onClick={onCancel}>
              Cancel
            </button>
