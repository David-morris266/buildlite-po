...
export async function listCertificateVariationCandidates(packageId,certificateId){const res=await fetch(buildUrl(`${certificatesUrl(packageId,certificateId)}/variation-candidates`));return handleJson(res);}
export async function reconcileAndAssessCertificateVariation(packageId,certificateId,applicationId,lineId,payload){const res=await fetch(buildUrl(`${certificatesUrl(packageId,certificateId)}/payment-applications/${encodeURIComponent(applicationId)}/variation-lines/${encodeURIComponent(lineId)}/assess`),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});return handleJson(res);}
