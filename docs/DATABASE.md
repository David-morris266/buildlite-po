...
**Last server contract banked:** **BL-038C — BANKED** (`b036bc9`; CE Expected Liability integrated into authoritative CVR Final Forecast).
**Last persistence slice implemented:** **BL-038E IMPLEMENTED; automated verification, human UAT, and SELECT-only forensic verification PASS; SAFE TO BANK.** Migration `022` creates schema-v3 Expected snapshot evidence and is applied to `buildlite_clone`. **Last design-authority slice:** **HD-038-1 / HD-038-2 / HD-038-3 RESOLVED** after BL-038A.
...
**NEXT:** **Bank BL-038E only after explicit approval.** BL-038D remains deferred pending pilot evidence. Do not silently start Hawthorn or another slice. Keep Test Site 1 P04 Draft. Do not create P05, alter Hawthorn, or start Detailed Selling Costs.
...
| `022_cvr_snapshot_expected_liability.sql` | Nullable Expected Liability on snapshot header/rows + nullable row provenance | **BL-038E IMPLEMENTED; HUMAN UAT + FORENSIC PASS; SAFE TO BANK.** New locks are schema v3 and require explicit Expected; old snapshots stay NULL/unavailable. Applied to `buildlite_test` and, as the sole controlled migration delta, `buildlite_clone`. |
...
**BL-038B** adds `021_commercial_event_expected_liability.sql`. Typed expected-liability treatment lives on `commercial_events`, not on `cvr_cost_code_inputs`. Default Expected is derived. **BANKED.** **BL-038C BANKED** at `b036bc9` and adds no migration: the close engine aggregates eligible submitted CE Expected per cost code, includes fact-only keys without overlay membership, and calculates Final = System + Expected + Adjustment. Snapshot schema/lifecycle, Prelims, Selling Costs, and Hawthorn remain unchanged. Human approval-transition UAT PASS: 5218 System £10k + Expected £20k → approval → System £30k + Expected £0; Final stayed £30k. SELECT-only forensic PASS: P01 Draft, zero P01 snapshots, no adjustment/accrual, Test Site 1 P04 Draft/P05 absent/snapshots 3.

**Post-038C pilot gate:** BL-038D CE/CVR drilldown/detail UX is deferred pending pilot evidence. BL-038E was the pre-Hawthorn correctness gate; automated transition/recomposition coverage, human UAT, and SELECT-only forensic verification pass. Detailed Selling Costs remains unstarted; Hawthorn remains protected and unmodified.

**BL-038E IMPLEMENTED / HUMAN UAT + FORENSIC PASS / SAFE TO BANK:** schema-v3 snapshot headers and rows persist Expected explicitly; rows also freeze submitted CE provenance. Historic reads never derive Expected from live CE state. Pre-v3 snapshots retain NULL Expected as unavailable. Create Next continues copying overlays only and recomputes Expected from live CE facts. Controlled clone evidence: P01 snapshot `66022ad0-7834-4c84-901f-4b1d3a367035` froze 5218 System £10k + Expected £20k + Adjustment £0 = Final £30k with CE-0025 submitted/default provenance; after CE approval, P02 `0fdf131b-0e29-4473-a09e-127a5172d3f3` has no overlay inputs and recomposes live to System £30k + Expected £0 = Final £30k. Historic P01 is unchanged. Test Site 1 retains three snapshots, P04 Draft, and no P05.
