...
**Last persistence slice implemented:** **BL-033D.x.2A.1 IMPLEMENTED AND BANKED** (`017_cost_codes_tenant_master.sql` on `buildlite_test` only; **not** on `buildlite_clone`). Server foundation exists; tenant Cost Code Master authority has **not** been cut over. **BL-033A design ACCEPTED.**  
...
**NEXT:** BL-033D.x.2A.2 (Administration Cost Codes server-authority UI, flag-controlled). Do not apply 017 to clone. Do not migrate the browser master. Do not turn authority ON. D.x.2 mapping remains blocked. Do not Submit or Approve & Lock P04. Do not create P05. Do not switch 5231 to TIME. Do not implement Review & Adopt / CVR adoption. Do not start D.x.3 instantiation.
...
| `017_cost_codes_tenant_master.sql` | Additive columns on `cost_codes` | **BL-033D.x.2A.1 IMPLEMENTED AND BANKED.** Admin-master fields + `version` + unique `(client_id, lower(btrim(code)))`. No backfill. No `element` → `description` copy. Applied on `buildlite_test` by tests. **Not** applied to `buildlite_clone`. Clone `lower(btrim(code))` collision check: **0**. `VITE_COST_CODE_SERVER_AUTHORITY` default OFF. Not an Admin authority cutover. |
...
- Cost-code **master records** remain browser `buildlite_cost_codes_master_v1` while `VITE_COST_CODE_SERVER_AUTHORITY` is OFF (default). BL-033D.x.2A.1 added server foundation; `017` is on `buildlite_test` only. Browser master has not been migrated. Admin still uses browser authority. D.x.2 mapping remains blocked. BL-033B classification metadata is **server-authoritative** (`cost_code_classifications`) and is not stored in that master.
