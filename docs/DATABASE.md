...
**Last persistence slice implemented:** **BL-038B BANKED.** Human UAT **PASS**. Forensic **PASS**. Close-engine Final Forecast still does **not** include Expected. **Last design-authority slice:** **HD-038-1 / HD-038-2 / HD-038-3 RESOLVED** after BL-038A (docs only). HD-001 / HD-002 / HD-008 remain resolved. Detailed Selling Costs remains unstarted. **BL-038B BANKED** (`021` on `buildlite_test` and local `buildlite_clone`). Close-engine Final Forecast still does **not** include Expected.  
...
**NEXT:** **BL-038C — integrate CE Expected Liability into authoritative CVR Final Forecast.** **Not started.** Keep P04 Draft. Do not Submit or Approve & Lock P04. Do not create P05. Do not Adopt UAT-CC-001. Do not claim Final Forecast includes Expected yet. Detailed Selling Costs remains unstarted. Do not import or remap Hawthorn. Do not switch 5231 to TIME. Do not Save migrated Admin rows whose server `reporting_group` is absent from the local Commercial Structure catalog.
...
- **HD-001 / HD-038 resolved:** System Forecast remains the approved/fact hierarchy above. **Authoritative future Final Forecast (HD-038-2):** System Forecast + CE Expected Liability + Commercial Adjustment. **BL-038B** stores treatment on `commercial_events` (`021`); default Expected is **derived** from submitted contract-value CE value (drafts contribute 0). Override/hold/exclude is stored separately from CE `value`. Expected may exceed submitted value (HD-038-3). Approval drops Expected by formula. Close-engine Final Forecast still ignores Expected until BL-038C. HD-002 replacement write remains proposal − System Forecast. Cost-code **identity** remains the client Master key; classifications are metadata.
...
**BL-038B** adds `021_commercial_event_expected_liability.sql`. Typed expected-liability treatment lives on `commercial_events`, not on `cvr_cost_code_inputs`. Default Expected is derived (not written on Submit). Typed prior/new expected columns on `commercial_event_audit`. Applied on `buildlite_test` by tests and on local `buildlite_clone` during controlled UAT prep. Close-engine money, snapshots, Prelims, Selling Costs, and Hawthorn are unchanged. Implementation PASS. Automated verification PASS. Human UAT **PASS**. Forensic **PASS**. **BANKED.** BL-038C is not started.
