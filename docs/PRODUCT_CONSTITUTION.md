...
| CVR | Draft → Submit → Approve & Lock; immutable snapshots (v2 includes Revenue; **BL-038E schema v3 freezes CE Expected + provenance**); QS accrual + commercial adjustment; carry-forward. **System Forecast** = approved commitment / budget / actual hierarchy. **Live authoritative Final Forecast:** System + CE Expected + Commercial Adjustment. Do not use accrual as CE expected liability. BL-038E implementation, human UAT, and forensic verification pass. |
...
**Current programme state:** **BL-038E implemented; automated, human UAT, and forensic PASS; SAFE TO BANK. NEXT: bank only with explicit approval.** BL-038D detail/drilldown UX is deferred pending pilot evidence. Do not silently start Hawthorn or another slice.
...
BL-037A  Authoritative Draft CVR membership command  BANKED
  ...
  → BL-038E snapshot/Create Next integrity                 IMPLEMENTED / UAT + FORENSIC PASS / SAFE TO BANK
...
**BL-034D is BANKED** at `6d11491`. Last design-authority bank is `b5fe388` (HD-038). **BL-038C BANKED** at `b036bc9`; human UAT + forensic PASS. **BL-038E is implemented; automated, human UAT, and forensic PASS; SAFE TO BANK. NEXT: bank only with explicit approval.** BL-038D detail UX is deferred pending pilot evidence. Do not silently start Hawthorn or another slice. Keep P04 Draft. Do not create P05. Do not Adopt UAT-CC-001. Hawthorn remains protected and unmodified. Detailed Selling Costs remains unstarted. HD-001, HD-002, HD-008, and HD-038-1/2/3 remain **resolved**.
