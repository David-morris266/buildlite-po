...
This document is the in-repo status snapshot for a clean Cursor session. It records the actual position after **Doc 67 persistence migration** through **BL-030**, **BL-ASUS-001**, **BL-031A–F**, **BL-032A**, **BL-032B**, **BL-032C**, **BL-032D**, **BL-033B**, **BL-033C**, **BL-033C.1**, **BL-033D.1**, **BL-033D.x.1**, and **BL-033D.x.2A.1**. **BL-031E is COMPLETE.** **BL-031F is COMPLETE.** **BL-032A is COMPLETE.** **BL-032B is COMPLETE.** **BL-032C is COMPLETE.** **BL-032D is COMPLETE.** **BL-033A design is ACCEPTED.** **BL-033B is COMPLETE.** **BL-033C is COMPLETE.** **BL-033C.1 is COMPLETE.** **BL-033D.1 is COMPLETE.** **BL-033D.x.1 is COMPLETE.** **BL-033D.x.2A.1 is IMPLEMENTED AND BANKED** (server foundation; tenant Cost Code Master authority has **not** been cut over). Test Site 1 development programme UAT **PASSED**. Test Site 1 classification UAT **PASSED** (`5231` Cleaning → PRELIMS + STANDARD_CVR; CVR money unchanged). Test Site 1 P03 is the first schema-v2 whole-CVR snapshot. Test Site 1 reporting-month UAT **PASSED**: P04 is **Draft** v1 with explicit `reporting_month` **2026-08**; P01/P02/P03 remain NULL. Test Site 1 Prelims UAT **PASSED**: calculation-only TIME / LUMP_SUM proposal, multiple lines per cost code, unresolved FIRST_COMPLETION, no CVR adoption. Company-template UAT **PASSED**: one company-owned copy of BuildLite Standard v1 (25 lines, unmapped, no £ defaults). `recognitionPolicy=exchange` is **not** live CVR/accounting behaviour. Repo authority-flag defaults remain OFF; local UAT used `client/.env.local` (do not commit it).
...
| Last persistence slice implemented | **BL-033D.x.2A.1 IMPLEMENTED AND BANKED.** Server foundation (`017_cost_codes_tenant_master.sql`) on `buildlite_test` by tests. **Not** applied to local `buildlite_clone`. Tenant Cost Code Master authority has **not** been cut over. |
...
| **NEXT** | **BL-033D.x.2A.2** — Administration Cost Codes server-authority UI, flag-controlled. Do **not** apply 017 to clone. Do **not** migrate the browser master. Do **not** turn authority ON in this bank. **D.x.2 mapping remains blocked.** Do **not** Submit or Approve & Lock P04. Do **not** create P05. Do **not** switch 5231 to TIME. Do **not** implement Review & Adopt / CVR adoption. Do **not** start D.x.3 development instantiation. Historic P01/P02/P03 `reporting_month` remains NULL. |
...
- Cost-code **master records** remain browser `buildlite_cost_codes_master_v1` while `VITE_COST_CODE_SERVER_AUTHORITY` is OFF (default). BL-033D.x.2A.1 added server foundation + additive `017` on `buildlite_test` only. Browser master has **not** been migrated. Admin Cost Codes still uses browser authority. D.x.2 mapping remains blocked. BL-033B classification metadata is **server-authoritative** (`cost_code_classifications`) and is not stored in the cost-code master.
...
`VITE_COST_CODE_SERVER_AUTHORITY` remains **OFF**. The live Admin Cost Codes screen still uses `buildlite_cost_codes_master_v1`. The browser master has **not** been copied. `017` is **not** applied to `buildlite_clone`. Clone `cost_codes` remains 95 rows in the old seven-column schema; `lower(btrim(code))` collision check: **0**. GET `/api/po/cost-codes` is unchanged as a compatibility view of the same table. Classification 013 remains independent. D.x.2 mapping remains blocked.

## Next action

**NEXT: BL-033D.x.2A.2** — switch Administration → Cost Codes onto server authority when `VITE_COST_CODE_SERVER_AUTHORITY=true`, without silently overwriting a richer browser master. Do **not** apply `017` to `buildlite_clone` until controlled x.2A.3 UAT. **D.x.2 company-template mapping remains blocked** until the tenant Cost Code Master is server-authoritative. Do **not** implement D.x.3 development instantiation. Do **not** implement Review & Adopt. Do **not** adopt Prelims into the CVR. P04 remains Draft. Do **not** Submit P04. Do **not** Approve & Lock P04. Do **not** create P05. Do **not** switch 5231 to TIME. Do **not** alter the three Test Site 1 Prelims UAT rows. Do **not** alter the 25-line BuildLite Standard v1.
